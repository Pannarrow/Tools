...
public class YTDUIDeviceHelper {
    private final static String TAG = "YTDUIDeviceHelper";
            ...
            try {
                ...
                if (matcher.find()) {
                    ...
                    if (versionString.length() > 0) {
                        ...
                        if (version.length >= 1) {
                            if (YTDParseUtil.parseInt(version[0]) < majorVersion) {
                                isLower = true;
                            }
                        }

                        if (version.length >= 2 && minorVersion > 0) {
                            if (YTDParseUtil.parseInt(version[1]) < majorVersion) {
                                isLower = true;
                            }
                        }

                        if (version.length >= 3 && patchVersion > 0) {
                            if (YTDParseUtil.parseInt(version[2]) < majorVersion) {
                                isLower = true;
                            }
                        }
                    }
    ...
    private static int getCoresFromFile(String file) {
        ...
        try {
            ...
            return YTDParseUtil.parseInt(num) + 1;
        } catch (IOException e) {
            return 0;
        } finally {
            try {
