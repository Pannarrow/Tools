...
public class YTDUIDisplayHelper {
    private static int screenWidth = -1;  //屏幕宽度
    private static int screenHeight = -1;  //屏幕高度
    ...
    private static final String TAG = "YTDUIDisplayHelper";
    ...
    public static int getScreenHeight(Context context) {
        if (screenHeight == -1) {
            screenHeight = getRealScreenHeight(context) - getNavigationBarHeight((Activity) context);
        }
        return screenHeight;
    }
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, YTDUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), YTDUINotchHelper.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && YTDUIDeviceHelper.isEssentialPhone()
                ...
                result -= 2 * YTDUIStatusBarHelper.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (YTDUIDeviceHelper.isHuawei() && !YTDUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
                result -= YTDUINotchHelper.getNotchSizeInHuawei(context)[1];
            }
    ...
    public static int getUsefulScreenHeight(Activity activity) {
        return getUsefulScreenHeight(activity, YTDUINotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenHeight(View view) {
        return getUsefulScreenHeight(view.getContext(), YTDUINotchHelper.hasNotch(view));
    }
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && YTDUIDeviceHelper.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * YTDUIStatusBarHelper.getStatusbarHeight(context);
...
//        }
    ...
    public static int getStatusBarHeight(Context context) {
        ...
        try {
            ...
            int x = YTDParseUtil.parseInt(field.get(obj).toString());
            if(x > 0){
    ...
    public static int getNavigationBarHeight(Activity activity) {
        ...
        for (int i = 0;i < vp.getChildCount();i++) {
            ...
            if (childView.getId() != View.NO_ID && NAVIGATION.equals(getCompatResourceEntryName(resources, childView.getId()))) {
                //底部状态栏是展示的，并返回其高度
    ...
    private static String getCompatResourceEntryName(Resources resources, int resId) {
        try {
     ...
     * @return
     */
    public static int getTextHeight(int maxWidth, String value, TextPaint paint) {
        StaticLayout staticLayout = new StaticLayout(value, paint, maxWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
